...
class Predictor:
    def __init__(self, config: ModelPredictionConfig):
        ...
        self.model.load_state_dict(
            torch.load(self.config.saved_model_path, map_location=self.device)
        )
        self.model.eval()
    ...
    def preprocess_text(self, text: str):
        ...
        numerical_input = [
            self.vocab.get(word, self.vocab.get("<UNK>", 0)) for word in text.split()
        ]
        return (
            torch.tensor(numerical_input, dtype=torch.long).unsqueeze(0).to(self.device)
        )
